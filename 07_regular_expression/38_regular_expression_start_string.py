...
Canada is a parliamentary democracy and a constitutional monarchy in the Westminster tradition. The country's head of government is the prime minister, who holds office by virtue of their ability to command 
the confidence of the elected House of Commons and is appointed by the governor general, representing the monarch of Canada, the ceremonial head of state. The country is a Commonwealth realm and is 
officially bilingual (English and French) in the federal jurisdiction. It is very highly ranked in international measurements of government transparency, quality of life, economic competitiveness, 
innovation, education and human rights. It is one of the world's most ethnically diverse and multicultural nations, the product of large-scale immigration. Canada's long and complex relationship with the 
United States has had a significant impact on its history, economy, and culture.
"""



result = re.search(r"^Canada.{20}", text, re.DOTALL | re.MULTILINE) # matches Canada at the start of the string

print(f"Match: '{result.group(0)}'") # matches the start of the string

# to match the canada at the center of the string
result = re.search(r".{20}Canada.{20}", text, re.DOTALL | re.MULTILINE) # matches Canada at the center of the string
print(f"Match: '{result.group(0)}'")


result = re.search(r".{20}France.{20}", text, re.DOTALL | re.MULTILINE) # matches France at the center of the string
print(f"Match: '{result.group(0)}'")